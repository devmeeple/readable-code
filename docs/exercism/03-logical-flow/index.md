...
해피 케이스, 긍정적인 부분을 바라보는 것이 자연스럽다. 하지만 개발을 할 때 예외 상황이 많다는 것을 고려해야 한다.
예를 들어 사용자 입력을 떠올린다. '비밀번호 8자 이상 입력하세요.'라는 안내 메시지가 있으면 8자 이상을 입력할 것 같지만 실제로는 그렇지 않다.
견고한 프로그램을 만들기 위해서는 여러 케이스를 고려해야 한다.

강의에서 `Optional` 객체와 `Null`을 어떻게 바라보는지 다룬다. `TypeScript`환경에서는 어떻게 적용하면 좋을지 고민하자.
