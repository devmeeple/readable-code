...
> 데이터라는 재료를 가지고 논리적인 코드를 작성하는 방법

## 3.1 뇌 메모리 적게 쓰기

코드를 읽고 이해, 인지하는데 비용이 소모된다. 비용을 줄이는 방법을 알아보자.

## 3.2 Early return

`return`(종료)을 빠르게 사용한다. `else` 사용빈도를 줄이고, 다음 문맥을 이해하는데 소모되는 비용을 낮춘다.

- *`else` 사용을 하지마세요가 아니다. 요구사항이 확실하면, 사용하는 빈도를 줄이자는 이야기다.
    - `switch`문과 같은 맥락이다.
