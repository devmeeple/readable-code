...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (doesUserChooseToPlantFlag(userActionInput)) {
                ...
                if (isLandMindeCell(selectedRowIndex, selectedColIndex)) {
                    board[selectedRowIndex][selectedColIndex] = "☼";
    ...
    private static boolean isLandMindeCell(int selectedRowIndex, int selectedColIndex) {
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 10; col++) {
                int count = 0;
                if (!isLandMindeCell(row, col)) {
                    if (row - 1 >= 0 && col - 1 >= 0 && isLandMindeCell(row - 1, col - 1)) {
                        count++;
                    }
                    if (row - 1 >= 0 && isLandMindeCell(row - 1, col)) {
                        count++;
                    }
                    if (row - 1 >= 0 && col + 1 < 10 && isLandMindeCell(row - 1, col + 1)) {
                        count++;
                    }
                    if (col - 1 >= 0 && isLandMindeCell(row, col - 1)) {
                        count++;
                    }
                    if (col + 1 < 10 && isLandMindeCell(row, col + 1)) {
                        count++;
                    }
                    if (row + 1 < 8 && col - 1 >= 0 && isLandMindeCell(row + 1, col - 1)) {
                        count++;
                    }
                    if (row + 1 < 8 && isLandMindeCell(row + 1, col)) {
                        count++;
                    }
                    if (row + 1 < 8 && col + 1 < 10 && isLandMindeCell(row + 1, col + 1)) {
                        count++;
                    }
